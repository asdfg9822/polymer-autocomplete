...
# autocomplete-plus's default prefix setting does not capture <. Manually check for it.
  ...
  buildLocalAttributeCompletion: (attribute, tag, options) ->
    ...
    attrObj = @completions.attributes[tag + "/" + attribute] || @completions.attributes[attribute]
  ...
  buildAttributeValueCompletion: (tag, attribute, value) ->
    completionObj = (@completions.attributes[tag + "/" +attribute] || @completions.attributes[attribute])
    if completionObj.global
      text: value
      type: 'value'
      description: completionObj.attribOptionDesc[value] || ""
